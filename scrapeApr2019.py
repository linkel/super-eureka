# This version of scrape scrapes the April 2019 calls. The other file grabs the Dec 2018 ones.
...
# variable for url
quote_page = 'https://finance.yahoo.com/quote/MSFT/options?date=1555545600'
apple_page = 'https://finance.yahoo.com/quote/AAPL/options?date=1555545600'
...
for row in parentlist:
    try:
        ...
        c.execute('INSERT INTO MSFTCallsApr2019 (ContractName, Date, Strike, LastPrice, Bid, Ask, Change, PChange, Volume, Openinterest, ImpliedVolatility) VALUES (?,?,?,?,?,?,?,?,?,?,?)', labels)
...
for row in parentlist2:
    try:
        labels = [row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10]]
        c.execute('INSERT INTO AAPLCallsApr2019 (ContractName, Date, Strike, LastPrice, Bid, Ask, Change, PChange, Volume, Openinterest, ImpliedVolatility) VALUES (?,?,?,?,?,?,?,?,?,?,?)', labels)
        #format(cn=row[0],dt=row[1],strike=row[2],lp=row[3],bid=row[4],ask=row[5],ch=row[6],pch=row[7],vol=row[8],oi=row[9],iv=row[10]))
    except sqlite3.IntegrityError:
        print('I don\'t have primary keys so this error shouldn\'t happen.')
